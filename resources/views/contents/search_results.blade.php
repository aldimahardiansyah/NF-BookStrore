...
@section('content')
    <div class="mt-3 mb-5">
        <h2 class="text-center">Cari <span class="fw-bold">{{ $judul }}</span></h2>
        <p class="text-center">Menampilkan total <span>{{ count($results) }}</span> hasil:</p>
    </div>
    <div class="container my-3">
        <div class="row row-cols-md-6 row-cols-sm-4 row-cols-2 g-4 rounded shadow-sm bg-white d-flex justify-content-center">
            @foreach ($results as $latest)
              <?php
